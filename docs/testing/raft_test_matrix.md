...
| Cross-compiler CommandBatch decode order | run `33316969138` closed the full macOS ARM64 gate and passed Ubuntu Clang public tests, SQLLogic, ASan/UBSan and Release process timelines, and TSan; only GCC public tests failed because two function arguments advanced one `ByteReader` in an order the C++ standard does not specify, so GCC read the body before the command type; decoding now sequences type, body and dispatch explicitly without changing bytes; on GCC 13 the independent handwritten nonempty V2 golden changed from a stable failure to codec 6/6, while real SQL prepare/apply/reopen passed 3/3 and three-node loopback passed 9/9; format, configured cpplint and direct tidy pass, and a fresh hosted workflow remains the final GCC oracle |
