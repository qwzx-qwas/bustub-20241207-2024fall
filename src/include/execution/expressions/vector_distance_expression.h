...
class VectorDistanceExpression : public AbstractExpression {
 public:
  VectorDistanceExpression(AbstractExpressionRef left, AbstractExpressionRef right, VectorDistanceExpressionType expr_type)
      : AbstractExpression({std::move(left), std::move(right)}, Column{"<val>", TypeId::DECIMAL}),
        expr_type_{expr_type} {
    if (GetChildAt(0)->GetReturnType().GetType() != TypeId::VECTOR || GetChildAt(1)->GetReturnType().GetType() != TypeId::VECTOR) {
      BUSTUB_ENSURE(GetChildAt(0)->GetReturnType().GetType() == TypeId::VECTOR && GetChildAt(1)->GetReturnType().GetType() == TypeId::VECTOR, "unexpected arg");
    }
  }

  auto Compute(const std::vector<double> &left, const std::vector<double> &right) const -> double {
    // TODO: implement l2_distance / cosine_distance / ip_distance.
    ...
    switch (expr_type_) {
      ...
      case VectorDistanceExpressionType::CosineDistance:
        ...
        {
            const double norm_l_sq = std::transform_reduce(
                left.begin(), left.end(), 0.0,
                std::plus<>(),
                [](double x) { return x * x; }
            );

            const double norm_r_sq = std::transform_reduce(
                right.begin(), right.end(), 0.0,
                std::plus<>(),
                [](double x) { return x * x; }
            );
  ...
  auto ToString() const -> std::string override {
    ...
    switch (expr_type_) {
        case VectorDistanceExpressionType::L2Distance: name = "l2_distance"; break;
        case VectorDistanceExpressionType::CosineDistance: name = "cosine_distance"; break;
        case VectorDistanceExpressionType::IPDistance: name = "ip_distance"; break;
    }
    //用 fmt::format（用于把模板字符串和参数拼成最终字符串）来格式化字符串，输出类似 "l2_distance(child0, child1)" 的结果
