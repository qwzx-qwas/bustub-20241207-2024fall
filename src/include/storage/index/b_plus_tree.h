...
class BPlusTree {
 ...
 public:
  ...
  // 执行叶子节点分裂
  auto HandleLeafSplit(WritePageGuard &leaf_guard, Context &ctx, const KeyType &new_key, const ValueType &new_value)
      -> void;

  // 将新节点插入到父节点中
  auto InsertIntoParent(WritePageGuard &parent_guard, const KeyType &key, WritePageGuard &child_guard, Context &ctx)
      -> void;

  // 执行内部节点分裂
  auto HandleInternalSplit(WritePageGuard &internal_guard, Context &ctx, const KeyType &new_key,
  ...
  auto HandleInternalUnderFlow(WritePageGuard &internal_guard, Context &ctx) -> void;
