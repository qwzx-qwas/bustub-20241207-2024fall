...
auto DeleteExecutor::Next([[maybe_unused]] Tuple *tuple, RID *rid) -> bool {
    ...
    for (auto index_info : indexes_) {
        ...
        auto index_key = old_tuple.KeyFromTuple(table_info_->schema_, *index_info->index_->GetKeySchema(),
        ...
        即逻辑删除
        */
        TupleMeta meta = table_heap_->GetTupleMeta(old_rid);
        meta.is_deleted_ = true;
        table_heap_->UpdateTupleMeta(meta, old_rid);
        delete_count++;
    }
    //构造输出tuple，包含删除的行数
