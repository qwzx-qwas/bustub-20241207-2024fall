...
void ExternalMergeSortExecutor<K>::GenerateRun(const std::vector<SortEntry> &run, std::vector<page_id_t> &pages) {
  ...
  SortPage *sort_page = reinterpret_cast<SortPage *>(guard.GetDataMut());
  ...
  for (const auto &entry : run) {
    ...
    // 尝试将元组写入当前排序页
    if (!sort_page->WriteTuple(t, tuple_size, slot_idx)) {
      ...
      slot_idx = 0;
      // 将元组写入新的排序页
      bool write_success = sort_page->WriteTuple(t, tuple_size, slot_idx);
...
auto ExternalMergeSortExecutor<K>::MergeRuns() -> std::optional<MergeSortRun> {
  // TODO: implement merge logic if needed, but for now we focus on Init
  return std::nullopt;
}
...
void ExternalMergeSortExecutor<K>::Init() {
  ...
  //所以这里要加上这个长度
  tuple_size = schema.GetInlinedStorageSize() + sizeof(uint32_t);
  ...
  // 计算每个 run 能容纳的最大元组数
  // K * SortPage::DATA_SIZE / tuple_size
  size_t max_tuples_per_run = (SortPage::DATA_SIZE * K) / tuple_size;
  ...
  while (child_executor_->Next(&tuple, &rid)) {
    ...
    if (current_run.size() >= max_tuples_per_run) {
      ...
      sorted_runs_.emplace_back(run_page_ids, tuple_size, exec_ctx_->GetBufferPoolManager());
  ...
  while (sorted_runs_.size() > 1) {
    ...
    for (size_t i = 0; i < sorted_runs_.size(); i += 2) {
      ...
      while (entry1.has_value() || entry2.has_value()) {
        ...
        // 写 Tuple 到 new run
        if (!sort_page->WriteTuple(selected_tuple, tuple_size, slot_idx)) {
          sort_page->SetTupleCount(slot_idx);
          current_page_id = exec_ctx_->GetBufferPoolManager()->NewPage();
          if (current_page_id == INVALID_PAGE_ID) {
             throw Exception("BufferPoolManager is full");
          ...
          new_run_pages.push_back(current_page_id);
          slot_idx = 0;
          sort_page->WriteTuple(selected_tuple, tuple_size, slot_idx);
        }
        slot_idx++;
      }
      sort_page->SetTupleCount(slot_idx);
      next_sorted_runs.emplace_back(new_run_pages, tuple_size, exec_ctx_->GetBufferPoolManager());
...
template <size_t K>
auto ExternalMergeSortExecutor<K>::MergeSortRun::Iterator::operator++() -> Iterator & {
  ...
  if (slot_idx_ >= static_cast<size_t>(sort_page_->GetTupleCount())) {
    ...
    if (page_idx_ < run_->pages_.size()) {
       page_guard_ = run_->bpm_->FetchPageRead(run_->pages_[page_idx_]);
       sort_page_ = page_guard_.As<SortPage>();
       slot_idx_ = 0;
    }
  }
  return *this;
...
template <size_t K>
auto ExternalMergeSortExecutor<K>::MergeSortRun::Iterator::operator*() -> Tuple {
  BUSTUB_ASSERT(sort_page_ != nullptr, "Dereferencing end iterator");
  return sort_page_->ReadTuple(run_->tuple_size_, slot_idx_);
}

template <size_t K>
auto ExternalMergeSortExecutor<K>::MergeSortRun::Iterator::operator==(const Iterator &other) const -> bool {
...
template <size_t K>
auto ExternalMergeSortExecutor<K>::MergeSortRun::Begin() -> Iterator {
  Iterator iter(this);
  if (!pages_.empty()) {
    iter.page_idx_ = 0;
    iter.slot_idx_ = 0;
    iter.page_guard_ = bpm_->FetchPageRead(pages_[0]);
    iter.sort_page_ = iter.page_guard_.As<SortPage>();
    
    // 处理空 run 的情况
     if (iter.sort_page_->GetTupleCount() == 0) {
        // 如果第一页就是空的，直接跳到 End
        iter.page_guard_.Drop();
...
template <size_t K>
auto ExternalMergeSortExecutor<K>::MergeSortRun::End() -> Iterator {
  Iterator iter(this);
  iter.page_idx_ = pages_.size();
  iter.slot_idx_ = 0;
  iter.sort_page_ = nullptr;
