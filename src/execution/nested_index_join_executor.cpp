...
auto NestIndexJoinExecutor::Next(Tuple *tuple, RID *rid) -> bool {
  ...
  while (true) {
    ...
    // 如果是 Left Join 且没有匹配的右表元组，产生一个 Right Tuple 为 Null 的连接元组
    // 注意：只有在 rid_index_ 为 0 即从未进入上方 while 循环发任何数据，且确实是个空结果集时才发
    // 但这里状态机很微妙：result_rids_.empty() 为真意味着刚才 ScanKey 没查到，或者已经发完了
    // 若要支持 Left Join，要在查完 Key 后立刻判断 empty

    // 2. 如果 result_rids_ 已经处理完，取下一个左表元组
    // 此时 rid_index_ == result_rids_.size()，说明上一轮的数据发完了
    // 但我们需要知道上一轮到底发没发过数据？如果是 LEFT JOIN 且没发过数据，得发一个
    // 由于状态机重置是在下面做的，所以无法区分“刚处理完一批”还是“刚查完发现是空的”
    // 所以逻辑需要微调：在 ScanKey 之后立刻判断是否 Empty
    ...
    // 通过索引查找右表 RIDs，填充到成员变量 result_rids_ 中
