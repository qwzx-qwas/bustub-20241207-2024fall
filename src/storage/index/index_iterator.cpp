...
INDEXITERATOR_TYPE::IndexIterator() : leaf_guard_(nullptr), current_leaf_page_(nullptr), page_id_(INVALID_PAGE_ID), index_(-1), bpm_(nullptr) {}

INDEX_TEMPLATE_ARGUMENTS
INDEXITERATOR_TYPE::IndexIterator(ReadPageGuard &&leaf_guard, const LeafPage *current_leaf_page, page_id_t page_id, int index, BufferPoolManager *bpm)
    : leaf_guard_(std::make_unique<ReadPageGuard>(std::move(leaf_guard))), current_leaf_page_(current_leaf_page), page_id_(page_id), index_(index), bpm_(bpm) {
